...
window.addEventListener("load", function () {
  ...
  $("button.btn-save-project").on("click", function () {
    if ($("div#xml-operation>input.project-name").val().length > 0) {
      ...
      Message.Show(
        '<p style="color:#666">【错误】保存项目</p><p>项目名不能为空</p>'
      );
    }
  });
  // 导出 Gradle 项目按钮
  ...
  // 加载项目界面 取消按钮
  $("div#projects-panel")
    .children("div.panel-content")
    .children("button.btn-cancel")
    .on("click", function () {
  ...
  $("a.friend-links").on("click", function () {
    Message.Show(
      '<p style="color:#666">友情链接</p><p>MiraiForum <a class="light-link" href="https://mirai.mamoe.net/" target="_blank">https://mirai.mamoe.net/</a></p>'
    );
  });
  // 链接-打开文件夹
